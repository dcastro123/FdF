...
/*   Updated: 2017/07/17 04:40:05 by dcastro-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_points	**store_cpoints(t_env *e)
{
	...
	while (++i < e->height)
	{
		...
		while (++j < e->width)
		{
			...
			point[i][j].z = e->store_arr[i][j] * 6;
			point[i][j].z2 = e->store_arr[i][j];
...
void	setup_env(t_env *e)
{
	...
	if (e->width > e->height)
		...
		e->dist = 700 / e->height;
	e->win_wmax = (e->width > e->height ? (800 + (e->width * e->dist)) : (800 + (e->height * e->dist)));
